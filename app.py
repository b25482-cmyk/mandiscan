...
def fetch_imd_rainfall():
    ...
    try:
        ...
        pass
    # Dynamic sandbox fallback configuration reading from state key safely
    return float(st.session_state.get("rain_slider_key", 0.0))
...
# Configuration profile dropdown placed above to drive widget configurations safely
...
# Base configuration initialization map
default_rain = 0
if sim_scenario == "Influenza Outbreak in Uhl Block during Heavy Rain":
    default_rain = 45

# FIXED: Standardized slider utilizing explicit keys, cleanly separated from data leaks
sandbox_rain_val = st.sidebar.slider("IMD Weather Station Rain (mm)", 0, 50, value=default_rain, key="rain_slider_key")

# Fetch operational rainfall cleanly
    ...
    with st.expander("📈 View Pathogen & Environmental Trend Analysis (Historical Metrics)"):
        ...
        if sim_scenario == "Norovirus Spike in Beas Block (Dry Weather)":
            for d in range(50, history_days):
                base_noro[d] *= (1.15 ** (d - 50)) # Norovirus surge curve
        elif sim_scenario == "Influenza Outbreak in Uhl Block during Heavy Rain":
            for d in range(55, history_days):
                base_flu[d] *= (1.12 ** (d - 55)) # Influenza surge curve
